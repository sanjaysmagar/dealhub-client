...
export default function AdminClient() {
  ...
  useEffect(() => {
    ...
    dispatch(
      fetchAdminDeals({ status, search: searchInput || undefined, limit: 50 }),
    );
    ...
    return (
      <div className={styles.wrap}>
        <div className={styles.lockedWrap}>
          ...
          <Link href="/" className={styles.lockedBtn}>
            Back to Home
          </Link>
        </div>
      </div>
    ...
    <div className={styles.wrap}>
      <div className={styles.header}>
        ...
        <div className={styles.subtitle}>
          Moderate and manage all deals on the platform
        </div>
      </div>
            ...
            <div key={deal._id} className={styles.dealRow}>
              ...
              <div className={styles.dealInfo}>
                ...
                <div className={styles.dealMeta}>
                  ...
                  <span className={styles.dealPoster}>
                    @{deal.postedBy?.username || "unknown"}
                  </span>
                  <span>{timeAgo(deal.createdAt)}</span>
                  <span
                    className={`${styles.statusPill} ${deal.status === "approved" ? styles.statusApproved : styles.statusRejected}`}
                  >
                  ...
                  {isExpired(deal) && (
                    <span className={styles.expiredPill}>
                      <Clock size={10} strokeWidth={3} /> Expired
                    </span>
                  )}
                </div>
              </div>
